...
uint32_t foobar_clk_time;
uint32_t foobar_last_clk_time;
int foobar_cur_clk;
...
static inline void select_cpu_clock_hw(int freq_idx, bool release_unused)
{
	...
	foobar_last_clk_time = t_end - t_begin;
	foobar_cur_clk = freq_idx;

	if (foobar_last_clk_time > foobar_clk_time)
		foobar_clk_time = foobar_last_clk_time;
...
{
	uint32_t platform_waiti_clock = CPU_WAITI_FREQ_IDX;
	int i;
	...
	for (i = 0; i < CONFIG_CORE_COUNT; i++) {
		sof->clocks[i] = (struct clock_info) {
			...
			.waiti_freq_idx = platform_waiti_clock,
